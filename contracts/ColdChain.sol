...
contract ColdChain {
    ...
    function issueCertificate(address _issuer, address _prover, string memory _status, bytes memory signature) public returns(uint) {
    ...
    function unmarshalMode(string memory _mode) private pure returns(Mode mode) {
        ...
        if(encodedMode == encodedMode0) return Mode.ISSUER;
        else if(encodedMode == encodedMode1) return Mode.PROVER;
        else if(encodedMode == encodedMode2) return Mode.VERIFIER;
    ...
    function unmarshalStatus(string memory _status) private pure returns(Status status) {
        ...
        bytes32 encodedStatus1 = keccak256(abi.encodePacked("DELIVERED"));
        bytes32 encodedStatus2 = keccak256(abi.encodePacked("STORED"));
        bytes32 encodedStatus3 = keccak256(abi.encodePacked("DISTRIBUTED"));
        bytes32 encodedStatus4 = keccak256(abi.encodePacked("DISPENSED"));

        if(encodedStatus == encodedStatus0) return Status.MANUFACTURED;
        else if(encodedStatus == encodedStatus1) return Status.DELIVERED;
        else if(encodedStatus == encodedStatus2) return Status.STORED;
        else if(encodedStatus == encodedStatus3) return Status.DISTRIBUTED;
        else if(encodedStatus == encodedStatus4) return Status.DISPENSED;
